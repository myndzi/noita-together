...
class NoitaTogetherWebsocket {
  ...
  private static async GetUserFromDB(id: string): Promise<User | null> {
    ...
    if (user) {
      const uaccessDataForUser = fs
        .readFileSync('.uaccess', 'utf-8')
        .replace(/[\n\r]/g, '\n')
        .split('\n')
    ...
    this.wsServer?.on('upgrade', async (req, socket: Socket, head) => {
      try {
        ...
        if (token.startsWith('deviceAuth')) {
          ...
          recorder.connected(socket, req.url);

          const user = await this.validateToken(token); //Get user from token

          if (!user) {
            console.log(`HTTP/1.1 401 Unauthorized`);
            socket.write('HTTP/1.1 401 Unauthorized\r\n\r\n');
            socket.destroy();
            recorder.failed(socket, 'Unauthorized');
            return;
          }

          Lobby.server.handleUpgrade(req, socket, head, (ws: any) => {
            recorder.upgraded(socket, ws, user.uaccess ?? 0)?.tap(ws);
